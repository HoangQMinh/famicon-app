...
function OtpVerifyContent() {
  ...
  // Countdown timer — OTP was already sent when the email page submitted
  useEffect(() => {
  ...
  function handleDigitChange(index: number, value: string) {
    // Strip non-digits; keep only the last character typed (handles mobile autocomplete)
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      try {
        ...
        // Session established — handle invite token if present, then navigate.
        if (inviteToken) {
          try {
            const inviteResult = await acceptInvite(inviteToken);
            if (inviteResult.success && inviteResult.data.is_returning_member) {
            ...
            // acceptInvite failed — fall through to default navigation
          }
        }

        // Default: navigate to home (client-side so session cookie is sent).
    ...
    <main className="auth-page">
      ...
      <div className="otp-resend-section">
        ...
        <button
          ...
          // Use aria-disabled rather than disabled so screen readers still read countdown
